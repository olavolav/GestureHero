class Hands
{
  ...
  void update_range_values() {
    for(int i=0; i<2; i++) {
      if(hands_array[i].x < min_x) { min_x = hands_array[i].x; }
      if(hands_array[i].x > max_x) { max_x = hands_array[i].x; }
      if(hands_array[i].y < min_y) { min_y = hands_array[i].y; }
      if(hands_array[i].y > max_y) { max_y = hands_array[i].y; }
      if(hands_array[i].z < min_z) { min_z = hands_array[i].z; }
      if(hands_array[i].z > max_z) { max_z = hands_array[i].z; }
    }
  }
