class GestureOutcomes
{
  ...
  float evaluate_performance() {
    float deviation_normal_order = outcomes[next_outcome_index].deviation_from_ideal(hands.hands_array[0].normalized_x(), 1.0 - hands.hands_array[0].normalized_y(), hands.hands_array[1].normalized_x(), 1.0 - hands.hands_array[1].normalized_y());
    // println("DEBUG: normal order deviation = "+deviation_normal_order);
    float deviation_swapped_order = outcomes[next_outcome_index].deviation_from_ideal(hands.hands_array[1].normalized_x(), 1.0 - hands.hands_array[1].normalized_y(), hands.hands_array[0].normalized_x(), 1.0 - hands.hands_array[0].normalized_y());
